...
let result = calculateArea(x: 5.0, y: 5.0)
if let value = result {
    print(value)
} else {
...
let result1 = calculateArea(x: -1, y: 5.0)
...
func add(x: Int?, y: Int?) -> Int? {
    guard let numX = x, let numY = y else {
        return nil
    }
 ...
 */
let randomUser = createUser()
if let user = randomUser {
    print("\(user.firstName) \(user.lastName) have \(user.age) years old.")
} else {
    print("Not valid!")
}
